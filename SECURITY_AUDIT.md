...
- **Deployed security headers: grade A+** (securityheaders.com, 2026-07-01), including a
  restrictive `Permissions-Policy`, HSTS (`max-age=31536000; includeSubDomains`), CSP,
...
**Deployed security headers verified via securityheaders.com: grade A (2026-06-19), since raised to
grade A+ (re-verified 2026-07-01 on the live site).** The only missing header was
`Permissions-Policy`, which is now set server-side (restrictive — opts out of
camera/microphone/geolocation/payment/USB/etc.), completing the header set and lifting the grade to
A+; this completes the previously-"partial" MED-02 header posture.
...
| MED-02  | Missing CSP/HSTS/security header posture                | ✅ Remediated  | Helmet enabled (CSP, frameguard, HSTS, etc.) + restrictive `Permissions-Policy` added server-side. Deployed headers verified via securityheaders.com: grade A (2026-06-19), raised to A+ (2026-07-01). | Security polish pass           |
