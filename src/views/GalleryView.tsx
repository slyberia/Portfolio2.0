...
const GALLERY_AI_ASSETS: AIVisualAsset[] = [
  {
    ...
    prompt: 'midcentury modern luxury penthouse interior, concrete plaster walls, warm glowing brass highlights, large architectural windows, ann arbor cityscape at dusk, architectural digest style, cinematic lighting --ar 16:9 --v 6.0',
    seed: 849301824,
    model: 'Midjourney v6.0',
    parameters: '--style raw --stylize 250 --v 6.0',
    aspect: '16:9',
    visualStyle: 'loft',
  },
  {
    ...
    prompt: 'Sleek futuristic operations control dashboard interface, complex node graph network, cybersecurity data visualization overlay, cybernetic color palette with amber and cyan accents, HUD layout, user interface design --ar 16:9 --v 6.0',
    seed: 194857104,
    model: 'Midjourney v6.0',
    parameters: '--stylize 150 --v 6.0 --ar 16:9',
    aspect: '16:9',
  ...
  },
  {
    ...
    prompt: 'Set of flat vector glyph icons for spatial and geographical information systems, clean golden ratio curves, minimalist architectural lines, deep slate background, vector art, behance style --ar 1:1 --v 6.0',
    seed: 90231847,
    model: 'DALL-E 3',
    parameters: 'Quality: Standard, Size: 1024x1024, Style: Vivid',
    aspect: '1:1',
    visualStyle: 'vector',
  },
];
...
const GALLERY_DIAGRAMS: TechnicalDiagram[] = [
  {
    ...
    description: 'Multi-layered system integration showing real-time websocket synchronization between the user client, Express gateway, and Google Gemini API with fallback vector database RAG retrieval.',
    diagramType: 'twin',
  },
  {
    ...
    description: 'PostGIS data ingestion pipeline that parses spatial coordinate packets, indexes them with R-Tree spatial indexes, and streams real-time updates via Server-Sent Events (SSE) to the React map layer.',
    diagramType: 'spatial',
  },
  {
    id: 'triage-state',
    ...
    description: 'Finite state machine governing the operational triage simulator game loop, handling user actions, tick updates, randomized failure events, and score evaluation.',
    diagramType: 'state',
  },
];
...
const GALLERY_LIVE_PROOFS: LiveProof[] = [
  {
    ...
    description: 'A custom developed professional replacement for the legacy Luxe Lofts brochure website, engineered with high contrast typography, optimized conversion pathways, and smooth layout interactions.',
  ...
  },
  {
    ...
    description: 'A fully interactive simulation dashboard where candidates and recruiters triage real-time server incidents, demonstrating complex state management, data visualization, and analytical reasoning.',
  ...
  },
  {
    ...
    description: 'A custom chatbot interface trained on Kyle Semple\'s career history, work samples, and professional philosophy, integrated with system-level context triggers and escalation fallback rules.',
    tech: ['Gemini API', 'Express.js Node Backend', 'Websockets', 'RAG Context Injection'],
    caseStudyId: 'digital-twin',
    metrics: ['Real-Time Streaming Responses', 'Custom Bot Guardrails', 'Context-Triggered Prompts'],
  },
];
      ...
      <div className="max-w-7xl mx-auto space-y-12">
        <header className="space-y-4 max-w-4xl">
          ...
          <p className="text-sm text-slate-600 dark:text-slate-300">
            Explore AI generation pipelines, zoom into interactive PostGIS & node system architectures, 
            or jump directly into live sandbox environments proving full stack readiness.
          </p>
        </header>
                ...
                <div className="p-5 space-y-3">
                  ...
                  <div className="grid grid-cols-2 gap-2 text-[11px] font-mono text-slate-600 dark:text-slate-400 border-t border-slate-100 dark:border-slate-800 pt-3">
                    <div>
                      ...
                      <span className="font-semibold text-slate-800 dark:text-slate-200">{asset.model}</span>
                    </div>
                    <div>
                      <span className="text-slate-400 block text-[9px] uppercase">Seed</span>
                      <span className="font-semibold text-slate-800 dark:text-slate-200">{asset.seed}</span>
                    </div>
              ...
              >
                ...
                <div className="relative h-48 bg-slate-950 border-b border-slate-200 dark:border-slate-800 overflow-hidden flex items-center justify-center p-4">
                  {diagram.diagramType === 'twin' && (
                    <svg className="w-full h-full text-tide-aqua/30" viewBox="0 0 100 100" fill="none">
                      <rect x="5" y="10" width="20" height="20" rx="3" stroke="currentColor" strokeWidth="1" />
                      <text x="7" y="22" className="text-[6px] font-mono fill-current font-bold">Client</text>
                      
                      <rect x="40" y="10" width="20" height="20" rx="3" stroke="currentColor" strokeWidth="1" />
                      <text x="42" y="22" className="text-[6px] font-mono fill-current font-bold">Gateway</text>
                      
                      <rect x="75" y="10" width="20" height="20" rx="3" stroke="currentColor" strokeWidth="1" />
                      <text x="77" y="22" className="text-[6px] font-mono fill-current font-bold">Gemini</text>
                      
                      <rect x="40" y="55" width="20" height="20" rx="3" stroke="currentColor" strokeWidth="1" />
                      <text x="42" y="67" className="text-[6px] font-mono fill-current font-bold">VectorDB</text>

                      {/* Connections */}
                      <path d="M25 20 H40" stroke="currentColor" strokeWidth="0.75" strokeDasharray="2" />
                  ...
                  {diagram.diagramType === 'spatial' && (
                    <svg className="w-full h-full text-tide-blue/30" viewBox="0 0 100 100" fill="none">
                      {/* Grid */}
                      <line x1="10" y1="0" x2="10" y2="100" stroke="currentColor" strokeWidth="0.25" strokeDasharray="1" />
                      <line x1="30" y1="0" x2="30" y2="100" stroke="currentColor" strokeWidth="0.25" strokeDasharray="1" />
                      <line x1="50" y1="0" x2="50" y2="100" stroke="currentColor" strokeWidth="0.25" strokeDasharray="1" />
                      <line x1="70" y1="0" x2="70" y2="100" stroke="currentColor" strokeWidth="0.25" strokeDasharray="1" />
                      ...
                      <text x="15" y="85" className="text-[5px] font-mono fill-current font-bold">R-Tree Spatial Index</text>
                    </svg>
                  )}

                  {diagram.diagramType === 'state' && (
                    <svg className="w-full h-full text-tide-cyan/30" viewBox="0 0 100 100" fill="none">
                      <circle cx="20" cy="50" r="10" stroke="currentColor" strokeWidth="1" />
                      <text x="14" y="52" className="text-[5px] font-mono fill-current font-bold">Idle</text>
                      
                      <circle cx="50" cy="50" r="10" stroke="currentColor" strokeWidth="1" />
                      <text x="44" y="52" className="text-[5px] font-mono fill-current font-bold">Active</text>
                      
                      <circle cx="80" cy="50" r="10" stroke="currentColor" strokeWidth="1" />
                      <text x="74" y="52" className="text-[5px] font-mono fill-current font-bold">Triage</text>

                      <path d="M30 50 H40" stroke="currentColor" strokeWidth="0.75" />
                      <path d="M60 50 H70" stroke="currentColor" strokeWidth="0.75" />
                      <path d="M75 41 C65 25, 35 25, 25 41" stroke="currentColor" strokeWidth="0.75" />
                    </svg>
                  )}
                ...
                <div className="space-y-4">
                  ...
                  <div className="space-y-1.5 border-t border-slate-100 dark:border-slate-800 pt-3">
                    ...
                    <ul className="space-y-1">
                      {proof.metrics.map((m) => (
                        <li key={m} className="flex items-center gap-2 text-[11px] text-slate-600 dark:text-slate-300">
          ...
          <div className="relative w-full max-w-5xl h-[85vh] flex flex-col bg-white dark:bg-[#0B0F19] border border-slate-200 dark:border-slate-800 rounded-xl overflow-hidden shadow-2xl">
            ...
            <div className="p-4 border-b border-slate-200 dark:border-slate-800 flex items-center justify-between">
              <div>
                ...
                <h2 id="modal-title" className="text-base font-bold text-ink-navy dark:text-white mt-0.5">
            ...
            >
              ...
              >
                {selectedDiagram.diagramType === 'twin' && (
                  <svg className="w-full h-full text-tide-aqua" viewBox="0 0 250 250" fill="none" stroke="currentColor" strokeWidth="1.5">
                    {/* User Agent */}
                    <rect x="10" y="80" width="60" height="40" rx="4" fill="#07161f" />
                    <text x="22" y="103" className="text-[7px] font-mono fill-current font-bold" stroke="none">User Browser</text>
                    <text x="18" y="112" className="text-[5px] font-mono fill-slate-500" stroke="none">React Client</text>

                    {/* Websocket Connection */}
                    <path d="M70 100 H110" stroke="#d8a84f" strokeWidth="1.5" strokeDasharray="3" />
                    <text x="75" y="94" className="text-[5px] font-mono fill-[#d8a84f]" stroke="none">Websocket (WS)</text>

                    {/* Express Backend */}
                    <rect x="110" y="80" width="60" height="40" rx="4" fill="#07161f" />
                    <text x="120" y="103" className="text-[7px] font-mono fill-current font-bold" stroke="none">Express Gateway</text>
                    <text x="118" y="112" className="text-[5px] font-mono fill-slate-500" stroke="none">Node backend API</text>

                    {/* Gemini Orchestrator */}
                    <rect x="180" y="80" width="60" height="40" rx="4" fill="#07161f" />
                    <text x="192" y="103" className="text-[7px] font-mono fill-[#39b8bc] font-bold" stroke="none">Gemini LLM</text>
                    <text x="188" y="112" className="text-[5px] font-mono fill-slate-500" stroke="none">Streaming REST</text>

                    {/* Database Pipeline */}
                    <rect x="110" y="160" width="60" height="30" rx="4" fill="#07161f" />
                    <text x="118" y="178" className="text-[7px] font-mono fill-current font-bold" stroke="none">Vector RAG DB</text>
                    ...
                    {/* Explanations */}
                    <rect x="10" y="210" width="230" height="30" rx="2" fill="#07161f" stroke="#39b8bc" strokeWidth="0.5" />
                    <text x="15" y="222" className="text-[5px] font-mono fill-slate-300" stroke="none">System Adherence: The Node gateway authenticates user sockets, queries</text>
                    <text x="15" y="232" className="text-[5px] font-mono fill-slate-300" stroke="none">Vector database RAG matches, constructs context payload, and streams back responses.</text>
                  </svg>
                )}

                {selectedDiagram.diagramType === 'spatial' && (
                  <svg className="w-full h-full text-tide-blue" viewBox="0 0 250 250" fill="none" stroke="currentColor" strokeWidth="1.5">
                    {/* Ingestion stream */}
                    <path d="M10 40 L60 40" stroke="currentColor" strokeWidth="1.5" />
                    <circle cx="60" cy="40" r="3" fill="currentColor" />
                    <text x="15" y="32" className="text-[6px] font-mono fill-current font-bold" stroke="none">Coordinate Ingest</text>

                    {/* PostGIS processing database */}
                    <rect x="80" y="20" width="90" height="70" rx="4" fill="#07161f" />
                    <text x="105" y="42" className="text-[7px] font-mono fill-current font-bold" stroke="none">PostGIS Server</text>
                    
                    {/* R-Tree diagram representation */}
                    <rect x="90" y="55" width="20" height="20" rx="1" stroke="currentColor" strokeWidth="0.75" />
                    <rect x="95" y="60" width="10" height="10" rx="1" stroke="currentColor" strokeWidth="0.5" />
                    <rect x="125" y="55" width="35" height="20" rx="1" stroke="currentColor" strokeWidth="0.75" />
                    <text x="94" y="82" className="text-[4px] font-mono fill-slate-500" stroke="none">Spatial R-Tree Indexing</text>

                    {/* Streaming channel */}
                    <path d="M170 55 H200" stroke="#3aab7a" strokeWidth="1.5" />
                    <text x="175" y="49" className="text-[5px] font-mono fill-[#3aab7a]" stroke="none">SSE Stream</text>

                    {/* Target Client */}
                    <rect x="200" y="35" width="40" height="40" rx="4" fill="#07161f" />
                    <text x="207" y="58" className="text-[6px] font-mono fill-current font-bold" stroke="none">React Map</text>

                    {/* explanatory panel */}
                    <rect x="10" y="210" width="230" height="30" rx="2" fill="#07161f" stroke="#7c6dd8" strokeWidth="0.5" />
                    <text x="15" y="222" className="text-[5px] font-mono fill-slate-300" stroke="none">Ingestion Flow: Coordinate coordinates are mapped against PostGIS boundary tables.</text>
                    <text x="15" y="232" className="text-[5px] font-mono fill-slate-300" stroke="none">R-Tree indexing aggregates polygons, instantly broadcasting updates to react-leaflet clients.</text>
                  </svg>
                )}

                {selectedDiagram.diagramType === 'state' && (
                  <svg className="w-full h-full text-tide-cyan" viewBox="0 0 250 250" fill="none" stroke="currentColor" strokeWidth="1.5">
                    {/* States circles */}
                    <circle cx="40" cy="80" r="20" fill="#07161f" />
                    <text x="32" y="83" className="text-[6px] font-mono fill-current font-bold" stroke="none">IDLE</text>

                    <circle cx="125" cy="80" r="20" fill="#07161f" />
                    <text x="115" y="83" className="text-[6px] font-mono fill-current font-bold" stroke="none">RUNNING</text>

                    <circle cx="210" cy="80" r="20" fill="#07161f" />
                    <text x="201" y="83" className="text-[6px] font-mono fill-current font-bold" stroke="none">TRIAGE</text>

                    <circle cx="125" cy="160" r="20" fill="#07161f" stroke="#d8a84f" />
                    <text x="110" y="163" className="text-[6px] font-mono fill-[#d8a84f] font-bold" stroke="none">EVALUATING</text>
                    ...
                    <path d="M105 160 H60 C40 160, 40 100, 40 100" stroke="currentColor" strokeWidth="1" strokeDasharray="3" />

                    {/* Explanatory text */}
                    <rect x="10" y="210" width="230" height="30" rx="2" fill="#07161f" stroke="#3aab7a" strokeWidth="0.5" />
                    <text x="15" y="222" className="text-[5px] font-mono fill-slate-300" stroke="none">State Rules: Active state tick periodically triggers randomized incident events.</text>
                    <text x="15" y="232" className="text-[5px] font-mono fill-slate-300" stroke="none">Triage logs actions. Evaluation computes score weights based on latency & system restore speed.</text>
                  </svg>
                )}
              </div>
            </div>
