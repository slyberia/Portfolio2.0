  ...
  useEffect(() => {
    const interval = setInterval(() => {
      ...
      const newIncident: Incident = {
        ...
        message:
          Math.random() > 0.5
            ? 'Geometry snapping failure detected'
            : 'Payload schema mismatch on ingress',
      };
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 divide-y lg:divide-y-0 lg:divide-x border-slate-800">
        ...
        <div className="p-6 space-y-8 col-span-1 lg:col-span-2">
          <div className="bg-slate-900 border border-slate-800 rounded-lg p-4">
            ...
            <div className="text-sm text-slate-300 leading-relaxed font-mono">{readout}</div>
          </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
            ...
            <div className="p-4 bg-slate-900/50 border border-slate-800 rounded-lg">
              ...
              <div
                className={`text-2xl font-bold ${mode === 'Throughput' ? 'text-rose-400' : 'text-emerald-400'}`}
              >
                {kpis.fpy}
              </div>
            </div>
            ...
            <div className="p-4 bg-slate-900/50 border border-slate-800 rounded-lg">
              ...
              <div
                className={`text-2xl font-bold ${mode === 'Quality' ? 'text-rose-400' : 'text-emerald-400'}`}
              >
                {kpis.backlog}
              </div>
            </div>
        ...
        <div className="flex flex-col bg-slate-950 col-span-1 lg:col-span-1 border-t lg:border-t-0 border-slate-800 h-96 lg:h-auto">
          ...
          <div className="flex-1 p-4 overflow-hidden relative">
            <div className="absolute inset-0 overflow-y-auto p-4 space-y-3 font-mono text-xs">
              {incidents.length === 0 && (
                <div className="text-slate-600 animate-pulse">Waiting for telemetry...</div>
              )}
              {incidents.map((inc, i) => (
                <div
                  key={i}
                  className="flex flex-col gap-1 border-l-2 pl-3 py-1 border-slate-700 animate-in fade-in slide-in-from-top-2 duration-300"
                  style={{ opacity: Math.max(0.2, 1 - i * 0.1) }}
                >
                  <div className="flex items-center justify-between text-[10px]">
                    <span className="text-slate-500">{inc.timestamp}</span>
                    <span
                      className={`px-1.5 py-0.5 rounded uppercase ${inc.status === 'PROCESSED' ? 'bg-emerald-500/10 text-emerald-400' : inc.status === 'BLOCKED' ? 'bg-rose-500/10 text-rose-400' : 'bg-amber-500/10 text-amber-400'}`}
                    >
                      {inc.status}
                    </span>
                  </div>
