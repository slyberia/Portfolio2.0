...
const ACCENT_STYLES = {
  implementation: {
    ...
    badge: 'bg-orange-50 border-orange-300 text-orange-800 dark:bg-orange-500/10 dark:border-orange-500/30 dark:text-orange-200',
  ...
  qa: {
    glow: 'bg-blue-500/10',
    badge: 'bg-blue-50 border-blue-300 text-blue-800 dark:bg-blue-500/10 dark:border-blue-500/30 dark:text-blue-200',
  ...
  gis: {
    glow: 'bg-teal-500/10',
    badge: 'bg-teal-50 border-teal-300 text-teal-800 dark:bg-teal-500/10 dark:border-teal-500/30 dark:text-teal-200',
    ...
    <div className="min-h-screen">
      <section className="relative pt-32 pb-16 px-6 overflow-hidden">
        ...
        <div className="max-w-5xl mx-auto space-y-6">
          <p className={`inline-flex items-center text-xs uppercase tracking-[0.25em] font-semibold border px-3 py-1.5 rounded-full ${accent.badge}`}>
            {content.eyebrow}
          </p>
        ...
        <div className="max-w-5xl mx-auto glass-card rounded-2xl p-8">
          ...
          <ul className="mt-5 grid grid-cols-1 md:grid-cols-2 gap-3">
            {content.proves.map((item) => (
              <li key={item} className="text-sm text-slate-700 dark:text-slate-300 leading-relaxed border border-black/5 dark:border-white/10 rounded-lg px-3 py-2 bg-white/80 dark:bg-slate-900/40">
                {item}
              </li>
            ))}
          </ul>
      ...
      <section className="px-6 py-12 bg-slate-50/60 dark:bg-slate-900/20 border-y border-black/5 dark:border-white/5">
        <div className="max-w-5xl mx-auto rounded-2xl border border-black/5 dark:border-white/10 bg-white/90 dark:bg-slate-900/60 p-8">
          <p className={`text-xs font-bold uppercase tracking-[0.3em] ${accent.section}`}>{content.guynodeLabel}</p>
          <h2 className="mt-3 text-2xl font-outfit font-semibold text-navy-900 dark:text-white">
            {content.guynodeTitle}
          </h2>
          <p className="mt-3 text-slate-600 dark:text-slate-300 leading-relaxed">{content.guynodeSummary}</p>
          <ul className="mt-5 space-y-2">
            {content.guynodeBullets.map((bullet) => (
              <li key={bullet} className="text-sm text-slate-700 dark:text-slate-300 flex items-start gap-2">
                <span aria-hidden="true" className={`mt-1.5 w-1.5 h-1.5 rounded-full ${content.accent === 'implementation' ? 'bg-orange-500' : content.accent === 'qa' ? 'bg-blue-500' : 'bg-teal-500'}`} />
                <span>{bullet}</span>
              </li>
            ))}
          </ul>
            ...
            {content.supportingEvidence.map((item) => {
              const card = (
                <article className="h-full rounded-xl border border-[#ddd7cd] dark:border-white/10 bg-white/95 dark:bg-slate-900/70 p-5">
                  ...
                  <p className="mt-3 text-sm text-slate-600 dark:text-slate-300 leading-relaxed">{item.relevance}</p>
                  {item.roleChips && (
                    <div className="mt-3 flex flex-wrap gap-1.5">
                      {item.roleChips.map((chip) => (
                        <span key={`${item.title}-${chip}`} className={`text-[11px] px-2 py-0.5 rounded-md border ${accent.chip}`}>
                          {chip}
                        </span>
                      ))}
                    </div>
              ...
              const evidenceHref = item.href ?? (item.title.includes('Guynode') ? GUYNODE_SYSTEM_HREF : undefined);
        ...
        <div className="max-w-5xl mx-auto space-y-5">
          ...
          <div className="flex flex-wrap gap-2">
            {content.skillsTools.map((skill) => (
              <span key={skill} className={`text-xs font-medium px-3 py-1.5 rounded-full border ${accent.chip}`}>
                {skill}
              </span>
            ))}
          </div>
      ...
      <section className="px-6 py-12 pb-24">
        <div className="max-w-5xl mx-auto glass-card rounded-2xl p-8">
          <h2 className={`text-xs font-bold uppercase tracking-[0.3em] ${accent.section}`}>{content.ctaTitle}</h2>
            ...
            {actions.map((action, index) => (
              <Link
                ...
                className={`inline-flex items-center gap-2 px-4 py-2 rounded-lg font-semibold text-sm transition-colors focus:outline-none focus-visible:ring-2 ${
                  index === 0
                    ? `text-white ${accent.cta}`
                    : `border ${accent.ctaOutline}`
                }`}
              >
                {action.label}
              </Link>
