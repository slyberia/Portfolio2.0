...
const constructContext = () => {
  ...
  const skillsStr = SKILL_GROUPS.map((s) => `- ${s.category}: ${s.items.join(', ')}`).join('\n');

  const caseStudiesStr = CASE_STUDY_REGISTRY.map((c) => `- ${c.title} (ID: ${c.id})`).join('\n');
...
const isEnabled = () => import.meta.env.VITE_GEMINI_ENABLED === 'true';
...
export const generateSpeech = async (text: string): Promise<string | undefined> => {
  ...
  try {
    ...
    const response = await ai.models.generateContent({
      ...
      contents: [
        {
          parts: [{ text: `Speak this in a professional, helpful, and confident tone: ${text}` }],
        },
      ],
      config: {
