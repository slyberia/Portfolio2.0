...
import { RigorCard, HtmlPreviewCard, ArtifactGallery, TradeoffLog } from '../components/CaseStudyComponents';
...
const EvidenceMap: React.FC<{ activeId: string; onSelect: (id: string) => void }> = ({ activeId, onSelect }) => {
  return (
       ...
       <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6 pb-6 border-b border-black/5 dark:border-white/5">
          <div className="flex items-center gap-3">
            <div className="flex items-center justify-center w-8 h-8 rounded-xl bg-indigo-600 text-white shadow-lg shadow-indigo-500/30">
                <svg xmlns="http://www.w3.org/2000/svg" className="w-4 h-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><path d="M9 11l3 3L22 4"/><path d="M21 12v7a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h11"/></svg>
            </div>
            <div>
              <h3 className="font-outfit font-bold text-navy-900 dark:text-white text-sm uppercase tracking-wide">Evidence Map</h3>
              <p className="text-[10px] text-slate-500 font-medium">Select a node to navigate the system.</p>
            </div>
          </div>
             ...
             return (
               ...
               >
                  ...
                  <div className="flex justify-between items-start mb-2">
                    ...
                    {isActive && <span className="w-1.5 h-1.5 bg-white rounded-full animate-pulse shadow-[0_0_8px_white]"></span>}
                  </div>

                  {/* Title */}
                  <div className={`font-outfit font-bold text-xs leading-tight mb-1 ${
                      isActive ? 'text-white' : 'text-navy-900 dark:text-white group-hover:text-indigo-600 dark:group-hover:text-indigo-400'
                  }`}>
                      {study.title.replace('Prompter Hub V9', 'Prompter Hub').replace('Project Aegis Protocol', 'Project Aegis')}
                  </div>
...
const CaseStudyView: React.FC<CaseStudyViewProps> = ({ onNavigateToHome, activeStudyId, onActiveStudyChange }) => {
    ...
    <section className="pt-32 pb-32 px-4 sm:px-6">
      <div className="max-w-7xl mx-auto">
        <div className="flex items-center justify-between mb-12">
          ...
          >
            <svg xmlns="http://www.w3.org/2000/svg" className="w-5 h-5 transition-transform group-hover:-translate-x-1" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="m15 18-6-6 6-6"/></svg>
            Back to Overview
          </button>
          ...
          {activeStudy && (
            <div className="flex flex-col items-end gap-3">
              ...
              >
                <svg xmlns="http://www.w3.org/2000/svg" className="w-4 h-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M12 6V3"/><path d="m4.9 8.9-2.1-2.1"/><path d="M2.5 18h3.7"/><path d="m19.1 8.9 2.1-2.1"/><path d="M15.3 18h6.2"/><path d="m5 18 3.5-3.5L12 18l3.5-3.5L19 18"/><path d="M12 18v3"/></svg>
              ...
              {voiceError && (
                <div className="animate-in fade-in slide-in-from-top-2 duration-300 flex items-center gap-2 px-4 py-2 rounded-xl bg-amber-500/5 border border-amber-500/20 text-amber-600 dark:text-amber-400 text-[10px] font-bold uppercase tracking-wider">
                   <svg xmlns="http://www.w3.org/2000/svg" className="w-3 h-3" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="12"/><line x1="12" y1="16" x2="12.01" y2="16"/></svg>
        ...
        <div className={`grid gap-8 lg:gap-16 items-start transition-all duration-500 ease-in-out ${
          isSidebarOpen ? 'md:grid-cols-[320px_1fr] lg:grid-cols-[380px_1fr]' : 'md:grid-cols-[60px_1fr]'
        }`}>
          
          {/* Navigation Rail */}
          <aside className="space-y-6 md:sticky md:top-32 h-fit transition-all duration-500 z-10">
            <div className="flex items-start justify-between">
                <div className={`space-y-2 transition-all duration-300 ${isSidebarOpen ? 'opacity-100' : 'opacity-0 w-0 overflow-hidden'}`}>
                    <h1 className="text-3xl font-outfit font-bold text-navy-900 dark:text-white whitespace-nowrap">Evidence Hub</h1>
                </div>
                <button 
                ...
                >
                    {isSidebarOpen ? <svg xmlns="http://www.w3.org/2000/svg" className="w-5 h-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="m11 17-5-5 5-5"/><path d="m18 17-5-5 5-5"/></svg> : <svg xmlns="http://www.w3.org/2000/svg" className="w-5 h-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="m13 17 5-5-5-5"/><path d="m6 17 5-5-5-5"/></svg>}
                </button>
            </div>

            <div className={`transition-all duration-300 ${isSidebarOpen ? 'opacity-100 translate-x-0' : 'opacity-0 -translate-x-4 pointer-events-none absolute'}`}>
                <div className="flex flex-wrap gap-2 py-2">
                    <button onClick={() => setActiveFilter('all')} className={`px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider transition-all border ${activeFilter === 'all' ? 'bg-navy-900 dark:bg-white text-white dark:text-navy-900 border-transparent' : 'text-slate-400 border-black/5 dark:border-white/10'}`}>All</button>
                    {(Object.keys(CATEGORY_LABELS) as CaseStudyCategory[]).map(cat => (
                        <button key={cat} onClick={() => setActiveFilter(cat)} className={`px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider border ${activeFilter === cat ? `${CATEGORY_COLORS[cat]} text-white border-transparent` : 'text-slate-400 border-black/5 dark:border-white/10'}`}>{CATEGORY_LABELS[cat]}</button>
                    ))}
                </div>
                <div className="grid gap-3 mt-4">
                    {filteredStudies.map(study => (
                        ...
                        >
                            <div className={`absolute left-0 top-0 bottom-0 w-1 ${activeStudyId === study.id ? CATEGORY_COLORS[study.category] : 'bg-slate-200 dark:bg-slate-800'}`} />
                            <h3 className={`font-outfit font-bold text-sm ${activeStudyId === study.id ? 'text-navy-900 dark:text-white' : 'text-slate-500'}`}>{study.title}</h3>
                            <p className="text-[11px] leading-relaxed text-slate-400 mt-1">{study.rationale}</p>
                        </button>
                    ))}
                </div>
            </div>
          ...
          <main className="min-w-0 md:pl-4 border-l border-black/5 dark:border-white/5 relative">
            <div className={`absolute -inset-2 bg-indigo-500/5 rounded-[3rem] transition-opacity duration-1000 pointer-events-none border border-indigo-500/10 ${isGlowActive ? 'opacity-100 scale-100' : 'opacity-0 scale-95'}`} aria-hidden="true" />
            
            <ErrorBoundary location="Documentation Hub">
              {!activeStudy ? (
                   <div className="glass-card p-12 rounded-3xl text-center"><p className="text-slate-500">Selection required.</p></div>
              ) : (
                <div className={`animate-in fade-in slide-in-from-right-4 duration-500 space-y-12 mx-auto w-full max-w-4xl`}>
                    ...
                    {activeStudy.artifacts && activeStudy.artifacts.length > 0 && <ArtifactGallery artifacts={activeStudy.artifacts} />}
                    {activeStudy.constraints && activeStudy.constraints.length > 0 && <TradeoffLog constraints={activeStudy.constraints} />}
                </div>
              )}
            </ErrorBoundary>
          </main>
