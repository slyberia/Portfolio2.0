...
const HomeView: React.FC<HomeViewProps> = ({ onNavigateToCaseStudy, onOpenContact }) => {
  ...
  const getCategoryColorClass = (category: string) => {
    if (category.includes('Strategic')) return 'hover:border-indigo-500/50 hover:bg-indigo-500/5 text-indigo-700 dark:text-indigo-400';
    if (category.includes('Data & Systems')) return 'hover:border-amber-500/50 hover:bg-amber-500/5 text-amber-700 dark:text-amber-400';
    if (category.includes('Stack')) return 'hover:border-emerald-500/50 hover:bg-emerald-500/5 text-emerald-700 dark:text-emerald-400';
    ...
    <>
      ...
      >
        ...
        <div className="max-w-5xl mx-auto flex flex-col items-center text-center space-y-8 relative z-10">
          ...
          <p className="text-lg md:text-xl text-slate-600 dark:text-slate-400 leading-relaxed max-w-3xl mx-auto animate-in fade-in slide-in-from-bottom-6 duration-1000 delay-200">
            Technical Customer Success and Solutions Enablement hybrid specializing in AI workflows, operational triage, and reliable data systems. 
            I partner with AI platform and SaaS teams to bridge the gap between model potential and enterprise-grade customer outcomes at scale.
          </p>
          <div className="flex flex-wrap gap-4 justify-center animate-in fade-in slide-in-from-bottom-8 duration-1000 delay-300">
            ...
            >
              Contact
              <svg xmlns="http://www.w3.org/2000/svg" className="w-5 h-5 group-hover/btn:translate-x-1 transition-transform" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M5 12h14" /><path d="m12 5 7 7-7 7" /></svg>
            </button>
            <button
      ...
      <section className="py-12 border-y border-black/5 dark:border-white/5 bg-white/30 dark:bg-slate-900/10 backdrop-blur-sm">
        <div className="max-w-7xl mx-auto px-6 grid grid-cols-2 md:grid-cols-4 gap-8">
          <TickerCard metric="100+" label="Weekly Tickets" tooltip="Managed enterprise-grade pressure with ~100+ Zendesk conversations weekly at Printful." targetId="exp-printful" />
          <TickerCard metric="$100k+" label="Client Tiers" tooltip="Experience handling high-stakes accounts where operational accuracy is critical for revenue." targetId="exp-printful" />
          <TickerCard metric="120+" label="SLA Requests/wk" tooltip="Validated high-volume throughput in utility datasets via strict triage protocols at Apex Systems." targetId="exp-apex" />
          <TickerCard metric="4+ Years" label="GIS Expertise" tooltip="Core technical foundation in spatial data systems and architectural design." targetId="foundation" />
        </div>
      </section>
        ...
        <div className="max-w-7xl mx-auto">
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-16">
            <div className="space-y-4">
              <h2 className="text-xs font-bold text-indigo-600 dark:text-indigo-500 uppercase tracking-[0.3em]">Trajectory</h2>
              <h3 className="text-4xl md:text-5xl font-outfit font-bold text-navy-900 dark:text-white">Career Experience</h3>
            </div>
            <p className="text-slate-500 dark:text-slate-400 max-w-md leading-relaxed">
              A track record of stabilizing operations and delighting enterprise stakeholders in data-heavy environments.
            </p>
          </div>
          ...
          <div className="space-y-8">
            {EXPERIENCE.map((exp, idx) => (
              <div
                key={idx}
                id={exp.company.toLowerCase().includes('printful') ? 'exp-printful' : exp.company.toLowerCase().includes('apex') ? 'exp-apex' : undefined}
      ...
      {/* Skills Grid */}
      <section id="skills" className="py-32 px-6 scroll-mt-24 transition-colors duration-500 relative">
        ...
        <div className="max-w-7xl mx-auto">
          <div className="text-center mb-12 space-y-4">
            <h2 className="text-xs font-bold text-indigo-600 dark:text-indigo-500 uppercase tracking-[0.3em]">Toolbox</h2>
            <h3 className="text-4xl md:text-5xl font-outfit font-bold text-navy-900 dark:text-white">Skills & Technologies</h3>
          </div>
          ...
          <div className="relative mb-20 px-4 py-8 rounded-[3rem] bg-indigo-500/[0.03] dark:bg-indigo-500/[0.05] border border-indigo-500/10 backdrop-blur-sm shadow-inner">
            <div className="absolute -top-3 left-1/2 -translate-x-1/2 px-4 py-1 bg-white dark:bg-slate-900 border border-indigo-500/20 rounded-full shadow-sm">
               <span className="text-[10px] font-bold text-indigo-600 dark:text-indigo-400 uppercase tracking-widest">Executive Impact Summary</span>
            </div>
              ...
              >
                ...
                <div className="flex flex-wrap gap-2">
                  {group.items.map((skill, i) => (
                    ...
                    >
                      {skill}
                      <svg xmlns="http://www.w3.org/2000/svg" className="w-3 h-3 opacity-0 group-hover/skill:opacity-100 transition-opacity" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M7 7h10v10"/><path d="M7 17 17 7"/></svg>
                    </button>
                  ))}
                </div>
              </div>
      ...
      <section id="foundation" className="py-32 px-6 scroll-mt-24 transition-colors duration-500">
        <div className="max-w-7xl mx-auto grid lg:grid-cols-2 gap-16">
          <div className="space-y-12">
            <div className="space-y-4">
              <h2 className="text-xs font-bold text-indigo-600 dark:text-indigo-500 uppercase tracking-[0.3em]">Foundation</h2>
              <h3 className="text-4xl font-outfit font-bold text-navy-900 dark:text-white">Education</h3>
              <div className="glass-card p-8 rounded-3xl mt-6">
                <h4 className="text-xl font-outfit font-bold text-navy-900 dark:text-white">B.A., Geography</h4>
                <p className="text-indigo-600 dark:text-indigo-400 font-medium font-outfit">Queen's University</p>
          ...
          <div className="space-y-8">
            <h3 className="text-4xl font-outfit font-bold text-navy-900 dark:text-white">Certifications</h3>
              ...
              {CERTIFICATIONS.map((cert, idx) => (
                ...
                >
                  <div className="w-12 h-12 bg-indigo-600/10 dark:bg-indigo-600/20 rounded-xl flex items-center justify-center text-indigo-600 dark:text-indigo-400 shrink-0">
                    <svg xmlns="http://www.w3.org/2000/svg" className="w-6 h-6" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M12 15V3" /><path d="m15 12-3 3-3-3" /><path d="M18 17.66A9 9 0 1 1 5.64 5.64" /><rect width="8" height="4" x="8" y="13" rx="1" /></svg>
                  </div>
                  <div>
                    <h5 className="font-outfit font-bold text-navy-900 dark:text-white text-sm">{cert.name}</h5>
