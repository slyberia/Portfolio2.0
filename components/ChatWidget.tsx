...
const ChatWidget: React.FC<ChatWidgetProps> = ({ onNavigate, onAction, onShowToast }) => {
  ...
  const parseAndExecuteCommands = (text: string) => {
    ...
    if (actionMatch && onAction) {
      ...
      if (onShowToast) {
        const msg = actionType === 'resume' ? 'AI Twin: Preparing Resume download...' : 'AI Twin: Opening contact portal...';
        onShowToast(msg);
      }
      onAction(actionType);
  ...
  const handleSend = async (textOverride?: string) => {
    ...
    try {
      ...
      setMessages(prev => [...prev, { role: 'model', text: "I'm currently unavailable. Please reach Kyle directly! <<ACTION:contact>>" }]);
    } finally {
      setIsTyping(false);
    }
  };
  ...
  const clearHistory = () => {
    const initial = [{ role: 'model', text: "Conversation history reset. How can I help you today?" }];
  ...
  return (
    <div className="fixed bottom-6 right-6 z-[90] flex flex-col items-end pointer-events-none transition-colors duration-500">
      ...
      >
        <div className="bg-indigo-600/5 dark:bg-indigo-600/20 p-4 border-b border-black/5 dark:border-white/5 flex justify-between items-center">
            <div className="flex items-center gap-3">
                ...
                <div>
                    <h3 className="text-navy-900 dark:text-white font-bold font-outfit text-sm">Kyle's Digital Twin</h3>
                    <p className="text-[10px] text-indigo-600 dark:text-indigo-300 uppercase tracking-wider font-medium">Verified Ops Agent</p>
                </div>
            </div>
            <div className="flex items-center gap-2">
                ...
                <button onClick={() => setIsOpen(false)} className="p-1.5 text-slate-400 hover:text-navy-900 dark:hover:text-white transition-colors">
                    <svg xmlns="http://www.w3.org/2000/svg" className="w-5 h-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M18 6 6 18"/><path d="m6 6 18 18"/></svg>
                </button>
            </div>
        </div>

        <div className="flex-1 overflow-y-auto p-4 space-y-4 chat-scroll bg-slate-50 dark:bg-slate-950/50" ref={scrollRef}>
                    ...
                    <div className={`max-w-[85%] rounded-2xl p-3 text-sm leading-relaxed ${
                        ...
                        {msg.role === 'model' ? (
                           <ReactMarkdown components={{
                               ...
                               a: ({node, ...props}) => <a {...props} className="text-indigo-600 dark:text-indigo-300 underline" target="_blank" />
                           }}>
                               {msg.text}
                           </ReactMarkdown>
                        ) : msg.text}
      ...
      >
        ...
        {isOpen ? (
             <svg xmlns="http://www.w3.org/2000/svg" className="w-6 h-6 rotate-90 transition-transform duration-300" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="m18 15-6-6-6 6"/></svg>
        ) : (
        ...
        <div className="absolute -top-1 -right-1 bg-white text-indigo-600 rounded-full p-0.5 border-2 border-white dark:border-slate-950 shadow-sm">
             <svg xmlns="http://www.w3.org/2000/svg" className="w-3 h-3" viewBox="0 0 24 24" fill="currentColor"><path d="M9.9 2.5C10.6 1.1 12.6 1.1 13.3 2.5L14.7 5.3C14.9 5.8 15.4 6.1 15.9 6.2L18.9 6.8C20.5 7.1 21.1 9 19.9 10.1L17.7 12.2C17.3 12.6 17.1 13.1 17.2 13.6L17.8 16.6C18.1 18.2 16.4 19.4 15 18.6L12.3 17.2C11.8 16.9 11.3 16.9 10.8 17.2L8.1 18.6C6.7 19.4 5 18.2 5.3 16.6L5.9 13.6C6 13.1 5.8 12.6 5.4 12.2L3.2 10.1C2 9 2.6 7.1 4.2 6.8L7.2 6.2C7.7 6.1 8.2 5.8 8.4 5.3L9.9 2.5Z"/></svg>
        </div>
      </button>
    </div>
  );
